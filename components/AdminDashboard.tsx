...
interface AdminDashboardProps {
  ...
  onSaveCompany: (employer: Employer) => void;
  onSaveJob: (
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [modalState, setModalState] = useState<{
    type: "job" | "employer" | "seeker" | null;
    item: any | null;
    mode: "add" | "edit" | null;
  }>({ type: null, item: null, mode: null });
  ...
  const openModal = (
    ...
    if (mode === "add") {
      ...
      if (type === "employer") modalItem = emptyCompany;
    }
    setModalState({ type, item: modalItem, mode });
  };
  const closeModal = () =>
    setModalState({ type: null, item: null, mode: null });

  const openDeleteConfirm = (
    type: "job" | "employer" | "seeker",
    item: Job | Employer | JobSeeker
  ) => {
    setDeleteConfirm({
      type,
      id: item._id,
      name:
        type === "job"
          ? (item as Job).title
          : (item as Employer | JobSeeker).name,
    });
  };
  ...
  console.log(
    "AdminDashboard render with jobs:",
    jobs,
    "employers:",
    employers,
    "seekers:",
        ...
        <TableSection title="Manage Jobs" onAdd={() => openModal("job", "add")}>
          {jobs.map((job) => (
            <TableRow
              key={job._id}
              title={job.title}
              subtitle={employers.find((c) => c.id === job.companyId)?.name}
            //   onEdit={() => openModal("job", "edit", job)}
              onDelete={() => openDeleteConfirm("job", job)}
            />
          ))}
        </TableSection>
        <TableSection
          title="Manage Employers"
          onAdd={() => openModal("employer", "add")}
        >
          {employers.map((employer) => (
            <TableRow
              key={employer._id}
              title={employer.name}
              subtitle={employer.website}
            //   onEdit={() => openModal("employer", "edit", employer)}
              onDelete={() => openDeleteConfirm("employer", employer)}
            />
          ))}
        </TableSection>
        <TableSection
          title="Manage Job Seekers"
          onAdd={() => openModal("seeker", "add")}
        >
          {seekers.map((seeker) => (
            <TableRow
              key={seeker._id}
              title={seeker.name}
              subtitle={seeker.email}
            //   onEdit={() => openModal("seeker", "edit", seeker)}
              onDelete={() => openDeleteConfirm("seeker", seeker)}
            />
          ))}
        </TableSection>
      </div>
...
const TableRow: React.FC<{
  ...
  <div className="flex justify-between items-center p-2 rounded hover:bg-gray-50/50">
    ...
    <div className="flex items-center space-x-2">
      {/* <button
        onClick={onEdit}
        className="text-primary hover:text-primary-focus p-2 rounded-full hover:bg-primary/10"
      >
