...
const handleSaveSeekerProfile = async (updatedSeeker: JobSeeker) => {
  try {
    ...
    setSeekers((prev) =>
      prev.map((s) => (s.id === savedSeeker._id || s.id === savedSeeker.id ? savedSeeker : s))
    );
...
const handleSaveCompanyProfile = async (updatedCompany: Employer) => {
  try {
    ...
    setCompanies((prev) =>
      prev.map((c) => (c.id === savedCompany._id || c.id === savedCompany.id ? savedCompany : c))
    );
...
//   const handleCompanySaveJob = async (
...
const handleCompanySaveJob = (newJob: Job) => {
  ...
  const handleAddBlogPost = async (content: string) => {
    ...
    if (currentUserRole === "SEEKER") {
      ...
      authorName = (currentUser as Employer).name;
      authorPhotoUrl = (currentUser as Employer).logo;
    }
  ...
  const handlePostReaction = async (
    ...
    const updatedPost = await api.addOrUpdateReaction(
      postId,
      currentUser.id,
      reactionType
    );
